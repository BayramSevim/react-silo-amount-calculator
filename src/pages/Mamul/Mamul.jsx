...
export default function DashboardPage() {
  ...
  const [selectedColor, setSelectedColor] = useState('#1679AB'); // Varsayılan renk
  const [searchValue, setSearchValue] = useState('');

  const fetchData = async () => {
    try {
      const res = await axios.get(`${GetAPIUrl()}/api/Silo/GetStokMamul`);
      if (res.data) {
  ...
  const updateSiloColor = async (silo) => {
    await axios
      .get(`${GetAPIUrl()}/api/Silo/UpdateStokMamulColor`, {
        params: {
          Id: silo.Id,
          Color: selectedColor
        }
              ...
              <TableBody>
                {filteredData.map((hammadde) => (
                  <TableRow key={hammadde.Id}>
                    <TableCell sx={{ textAlign: 'center' }}>{hammadde.StokNo}</TableCell>
